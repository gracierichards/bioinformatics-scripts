...
output3 = open("genome_wide_correlations2.txt", "w")
...
for cluster in gene_cols:
	column = []
	#Keys are phage2 gene, values are correlation values
	correlations_for_orthogroup = {}
	for chunk in snp_annot_cols:
		...
		correlations_for_orthogroup[chunk] = abs(cell_value)
	
	most_correlated_chunk = max(correlations_for_orthogroup, key=correlations_for_orthogroup.get)
	correlation = correlations_for_orthogroup[most_correlated_chunk]
	if abs(correlation) >= 0.6:
		chunk1, chunk2 = most_correlated_chunk.split("-")
		phage2gene_1 = ""
		phage2gene_2 = ""
		...
		if phage2gene_1 == "" and phage2gene_2 == "":
			phage2gene = most_correlated_chunk
		...
		output3.write(cluster + "\t" + gene_function.replace("\t", " ") + "\t" + phage2gene + "\t" + str(correlation) + "\t" + epa_gene + "\n")
